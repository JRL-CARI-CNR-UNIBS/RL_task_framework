...
if __name__ == '__main__':
    ...
    if rospy.has_param('/skills_executer/skills_parameters_name_space'):
        exec_params_ns = rospy.get_param('/skills_executer/skills_parameters_name_space')
    else:
        rospy.logerr('No /skills_executer/skills_parameters_name_space param')
                                ...
                                for action_param_name in action.keys():
                                    if (action_param_name == 'skills'):
                                        for skill_name in action['skills'].keys():
                                            ...
                                            elif type(skill) is int:
                                                key = '/'+exec_params_ns+'/'+action_name+'/'+skill_name
                                                if key not in pub_dict:
                                                    pub_dict[key] = rospy.Publisher(key, Float32, queue_size=1)
                                                pub_dict[key].publish(skill)
                                            elif type(skill) is dict:
